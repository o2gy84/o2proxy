...
void EpollEngine::changeEvents(Client *c, engine::event_t events)
{
    ...
    if (!ret)
    {
        kvloge("epol_ctl error on sd: ", c->_sd);
    }
...
void EpollEngine::eventLoop()
{
    ...
    while (!Engine::g_Stop)
    {
        ...
        for (int i = 0; i < epoll_ret; ++i)
        {
            ...
            if (events[i].data.fd == listener())
            {
                if (false == accept_action(_epoll_fd, listener(), this))
                {
                    ...
                    kvloge("FATAL. accept: ", strerror(errno));
                    exit(-1);
                }
                continue;
            }
            ...
            else if (events[i].events & EPOLLIN)
            {
                ...
                {
                    kvlogd3("read EOF, disconnect on sd: ", c->_sd);
                    disconnected_clients.push_back(c);
                }
                else if (r > 0)
                {
            ...
            else
            {
                ...
                if (events[i].events & EPOLLERR)
                    kvloge("event epollerr on sd: ", cs->_sd);
                else
        ...
        {
            kvlogd2("GC start. disconnected clients: ", disconnected_clients.size());
