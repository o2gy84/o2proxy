...
namespace 
{
    ...
    int non_blocked_connect(const std::string &host, int port)
    {
        ...
        {
            ::close(sd);
            kvloge("connect error: ", strerror(errno));
            return -1;
        }
        return sd;
    }
...
void ProxyClient::onRead(const std::string &str)
{
    ...
    if (_state == state::INIT_STATE)
    {
        ...
        //kvlogd4("new request dump: ", _req.dump());
        ...
        if (host.empty())
        {
            kvloge("host is empty. close connection. request dump: ", _req.dump());
        ...
        if (_req.headers()._method == "CONNECT")
        {
            ...
            {
                kvloge("error add to event loop on connect: ", _req.dump());
        ...
        kvloge("unknown http request. close connection. request dump: ", _req.dump());
        shutdown(_sd, SHUT_RDWR);
        return;
    }
...
void ProxyClient::onDead()
{
    ...
    kvlogd3("shutdown partner socket: ", _partner->sd());
    if (_partner->sd() != -1)
    {
